...
class GRUCell(nn.Module):
    ...
    def reset(self, batch_size=1, cuda=True):
        ############################################# FOR STUDENTS #####################################
        # Reset recurrent states for the GRU Neural network (single recurrent state h_t)
        if cuda:
            self.recurrent_state = Variable(torch.zeros(batch_size, self.hidden_size)).cuda().double()
        else:
            self.recurrent_state = Variable(torch.zeros(batch_size, self.hidden_size)).double()
    #################################################################################################
