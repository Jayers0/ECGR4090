...
BotSpeed = 10  # Speed Of The Bot
model = Net(InputSize, HiddenSize,NumClasses)
...
def randpos():
    ...
    if(selector==0):
        env.Bot.position = Vec2d(100, 100)
        env.Bot.angle = math.pi/2
    elif(selector==1):
        env.Bot.position = Vec2d(100, 300)
        env.Bot.angle = math.pi/2
    elif(selector==2):
        env.Bot.position = Vec2d(100, 450)
        env.Bot.angle = math.pi/2
    else:
        env.Bot.position = Vec2d(500, 50)
        env.Bot.angle = 20
